...
>
> 本路线图是 `2026-05-09-langgraph-single-city-mvp.md` 的可执行版,把"治理已完成、代码迁移待开始"的现状切成 9 份独立可测、独立可提交的子计划。
>
> 按顺序执行,每份子计划完成后再开下一份。
...
- `Makefile` 或顶层 `npm run regression`:跑全套(typecheck + lint + unit + pytest + e2e)
...
- [ ] 浏览器手动走完整流程(本地起 web + api)成功
- [ ] `npm run regression` 跑通(包含 Playwright fixture 模式 e2e)
- [ ] 顶层 README 更新"如何启动 / 如何跑回归"段落

...
>
> Plan 1-9 are complete. Post-roadmap hardening continues in Plan 10 (launch readiness), Plan 11 (fixture API smoke gate), Plan 12 (roadmap closure), and Plan 13 (web dependency hygiene).
...
**改写**:
- `web/src/lib/apiClient.ts` — 强制 `NEXT_PUBLIC_API_URL` 必填,默认 `http://127.0.0.1:8000`,删 `discoverDestination` / `generatePlan`,新增匹配 Plan 6 的方法
